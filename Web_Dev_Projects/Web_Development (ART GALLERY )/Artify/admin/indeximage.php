<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $db->prepare('SELECT image FROM arts WHERE id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("../img/art/".$imgRow['image']);

		// it will delete an actual record from db
		$stmt_delete = $db->prepare('DELETE FROM arts WHERE id =:uid');
	...
	<main class="cd-main-content">
		...
		<div class="content-wrapper">
			...
			<?php
				...
				$stmt = $db->prepare('SELECT id, alt, image FROM arts ORDER BY id DESC');
				$stmt->execute();
				...
				if($stmt->rowCount() > 0)
				{
					while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					{
						...
						?>
						<div class="col-xs-3">
							<p class="page-header"><?php echo $alt; ?></p>
